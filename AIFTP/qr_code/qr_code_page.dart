...
class _QRViewPageState extends State<QRViewPage> {
  ...
  void _onQRViewCreated(QRViewController controller) {
    ...
    controller.scannedDataStream.listen((scanData) async {
      if (scanData.code != null && scanData.code!.isNotEmpty) {
        ...
        checkStatusByQR(scanResult: scanData.code);
       /* var result = await dashboardController.scanDetailResponse(context,scanData.code);
        if (result['status']) {
          isFound=false;
       ...
       //dashboardController.scanDetailResponse(context,scanData.code);
      }
    });
  }
  ...
  _entryQueryDialog(BuildContext context)async  {
    errorMsg("");
    ...
    if(data!=null && data.isNotEmpty){
      checkStatusByQR(scanResult: data);
    }
  }

  Future checkStatusByQR({scanResult}) async {
    editController.text="";
    if(scanResult.isNotEmpty){
      var result = await dashboardController.scanDetailResponse(context,scanResult);
      controller!.resumeCamera();
      if (result['status']) {
        Get.offNamed(UserListPage.routeName);
      }else{
