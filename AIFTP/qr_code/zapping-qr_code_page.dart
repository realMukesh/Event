...
class _ZappingQRPageState extends State<ZappingQRPage> {
                      ...
                      children: [
                        Row(
                          children: [
                            ...
                            SizedBox(width: 6,),
                            RegularTextView(text: "QR Scan",color: white,)
  ...
  Widget _buildQrView(BuildContext context) {
    ...
    var scanArea = (MediaQuery.of(context).size.width < 400 || MediaQuery.of(context).size.height < 400) ? 300.0 : 350.0;
  ...
  void _onQRViewCreated(QRViewController controller) {
    ...
    controller.scannedDataStream.listen((scanData) async {
      if (scanData.code != null && scanData.code!.isNotEmpty) {
        ...
        searchQR(scanResult: scanData.code);
  ...
  _EnterQueryDialog(BuildContext context)async  {
          ...
          child: SingleChildScrollView(
            child: Column(
              children: [
                ...
                const BoldTextView(
                  text: 'Search by unique code, email, name,mobile',
                  textSize: 12,
                ),
                const SizedBox(
                  height: 6,
                ...
                InputFormFieldCode(
                  ...
                  hintText: 'Enter unique code, email, name,mobile',
                  maxLength: 50,
                  validator: (String? value) {
                    if (value!.trim().isEmpty || value.trim() == null) {
                      errorMsg("Please unique code, email, name,mobile");
                      return 'Please unique code, email, name,mobile';
                    } else {
                      return null;
                    }
                  },
    ...
    if(data!=null && data.isNotEmpty){
      checkUserByCode(scanResult: data);
    }
  }

  Future searchQR({scanResult}) async {
    ...
    if (scanResult.isNotEmpty) {
      var result=await _zappingController.entryZapping(context, code: scanResult,location: _authManager.getLocation());
      if(result['status']){
  ...
  /*Future checkUserByCode({scanResult}) async {
    editController.text="";
    if (scanResult.isNotEmpty) {
      var result=await _zappingController.entryZapping(context, code: scanResult,location: _authManager.getLocation());
      print(result['status']);
      if(result['status']){
        var screenResult = await Get.to(ConfirmDialogPage(isSuccess: true,message: result['message']));
      }else{
        var screenResult = await Get.to(ConfirmDialogPage(isSuccess: false,message: result['message']));
      }
    }
  }*/
  ...
  Future checkUserByCode({scanResult}) async {
    editController.text="";
    if(scanResult.isNotEmpty){
      var result = await _zappingController.scanDetailResponse(context,scanResult);
      if (result['status']) {
        Get.offNamed(ZappingUserListPage.routeName);
      }else{
